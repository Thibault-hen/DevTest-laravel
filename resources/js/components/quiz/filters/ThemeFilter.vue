...
const isThemeSelected = (theme: string | null | undefined): boolean => (theme ? selectedThemes.value.includes(theme) : false);
...
<template>
    <div>
        <div>
            <HeadingSmall title="Thème" description="Filtrer par thème">
                <template #label>
                    <Badge v-if="selectedThemes.length > 0" class="border border-border bg-background text-black dark:text-white">
                        {{ selectedThemes.length }}
                        <span @click="resetThemes" class="cursor-pointer rounded-lg p-0.5 hover:bg-primary/50"><X :size="14" /></span>
                    </Badge>
                </template>
            </HeadingSmall>
        </div>
        <div class="mt-4 flex flex-col gap-2">
            <div v-for="theme in collapsedThemes" :key="theme?.id" class="flex items-center space-x-3">
                <Checkbox
                    v-if="theme?.title"
                    :id="theme.id"
                ...
                />
                <Label :for="theme?.id" class="flex cursor-pointer items-center gap-2">
                    {{ theme?.title }}
