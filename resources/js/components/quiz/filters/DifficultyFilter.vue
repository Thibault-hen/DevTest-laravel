  ...
  <div>
    ...
    <div class="mt-4 flex flex-col gap-2">
      <Button
        ...
        variant="secondary"
        class="cursor-pointer border font-bold uppercase"
        :class="{
          'opacity-100': selectedDifficulty.includes(difficulty.level),
        }"
        :style="{
          backgroundColor: selectedDifficulty.includes(difficulty.level) ? difficulty.color : `${difficulty.color}15`,
      ...
      >
        {{ difficulty.level }}
        <Badge class="ml-auto border border-border bg-background text-black dark:text-white">{{
          difficulty.quizzes_count
        }}</Badge>
      </Button>
    </div>
  </div>
