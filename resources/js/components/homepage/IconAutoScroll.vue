...
const logos: TechLogo[] = [
    { name: 'PHP', src: isDarkMode ? 'assets/img/logos/php_dark.svg' : 'assets/img/logos/php.svg', alt: 'PHP Logo', color: '#8892BF' },
...
<template>
    <Card class="relative mx-auto mt-10 w-full overflow-hidden rounded-lg p-4 shadow-md">
        <div class="relative z-0 py-4">
            <div class="animate-marquee-horizontal">
                <div v-for="(logo, index) in logos" :key="`first-${index}`" class="framework-card mr-12 inline-flex sm:mr-20">
                    <img :src="logo.src" :alt="logo.alt" class="h-6 w-6 object-contain md:h-12 md:w-12" :style="{ '--glow-color': logo.color }" />
                </div>

                <div v-for="(logo, index) in logos" :key="`second-${index}`" class="framework-card mr-12 inline-flex sm:mr-20">
                    <img :src="logo.src" :alt="logo.alt" class="h-6 w-6 object-contain md:h-12 md:w-12" :style="{ '--glow-color': logo.color }" />
                </div>
            </div>
        </div>

        <div class="pointer-events-none absolute top-0 left-0 z-10 h-full w-20 bg-gradient-to-r from-card via-card to-transparent"></div>
        <div class="pointer-events-none absolute top-0 right-0 z-10 h-full w-20 bg-gradient-to-l from-card via-card to-transparent"></div>
    </Card>
</template>
