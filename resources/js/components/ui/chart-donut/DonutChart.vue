...
const props = withDefaults(defineProps<Pick<BaseChartProps<T>, "data" | "colors" | "index" | "margin" | "showLegend" | "showTooltip" | "filterOpacity"> & {
  ...
  customTooltip?: Component
    /**
   * Donut chart arc width.
   */
  donutArcWidth?: number
}>(), {
...
<template>
  <div :class="cn('w-full h-48 flex flex-col items-end', $attrs.class ?? '')">
    <VisSingleContainer :style="{ height: isMounted ? '100%' : 'auto' }" :margin="{ left: 0, right: 0 }" :data="data">
      <ChartSingleTooltip
      ...
      class="text-red-500"
        ...
        :color="colors"
        :arc-width="type === 'donut' ? (donutArcWidth ? donutArcWidth : 20) : 0"
