...
import { dashboard, home, login, logout, quizzes, register } from '@/routes';
...
<template>
    <div class="relative flex flex-col items-center p-2 lg:justify-center lg:p-4">
        ...
        <div v-if="isOpen" class="fixed inset-0 z-40 backdrop-blur-sm lg:hidden" @click="toggleSideNav()" />
            ...
            <nav class="flex items-center justify-between gap-4 p-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <template v-if="!$page.props.auth.user">
                        <Link
                            :href="login()"
                            class="flex cursor-pointer items-center gap-1.5 rounded-[0.375rem] border bg-primary px-3 py-1.5 text-sm leading-normal text-white transition-colors duration-300"
                        >
                        ...
                        <Link
                            :href="register()"
                            class="flex cursor-pointer items-center gap-1.5 rounded-[0.375rem] border px-3 py-1.5 text-sm leading-normal transition-colors duration-300 hover:bg-primary hover:text-white"
                        >
                    ...
                    <Separator orientation="vertical" class="mr-4 hidden !h-6 lg:flex" />
                    <ThemeToggle />
                    <Separator orientation="vertical" class="mx-4 hidden !h-6 lg:flex" />
                    <NavUser v-if="$page.props.auth.user" :user="$page.props.auth.user" />

                    <!-- Hamburger menu mobile -->
                    <Button class="flex cursor-pointer rounded-md lg:hidden" size="sm" @click="toggleSideNav()">
                        <Menu v-if="!isOpen" />
                        <X v-else />
                    </Button>
                </div>
        ...
        >
            ...
            <div class="my-2.5 flex flex-col gap-1 overflow-y-auto px-2">
                ...
                >
                    <LayoutGrid :stroke-width="2" class="mr-2 h-4 w-4" />
            ...
            <div class="flex flex-col gap-1 overflow-y-auto border-t px-2 pt-2">
                <template v-if="!$page.props.auth.user">
                    ...
                    >
                        <LogIn :stroke-width="2" class="mr-2 h-4 w-4" />
                        Connexion
                    </Link>
                    <Link
                        :href="register()"
                    ...
                    >
                        <UserRoundPlus :stroke-width="2" class="mr-2 h-4 w-4" />
                        Inscription
                    </Link>
                </template>
                <Link
                ...
                >
                    <LogOut :stroke-width="1" class="mr-2 inline-block h-4 w-4 rotate-180" />
